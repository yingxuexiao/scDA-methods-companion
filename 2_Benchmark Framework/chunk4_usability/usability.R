...
### usability data
usability_data <- openxlsx::read.xlsx("/Users/xiaoying/Downloads/method_scoring.xlsx", rowNames = TRUE)
colnames(usability_data) <- gsub(x = colnames(usability_data), pattern = "[.]", replacement = " ")

### weight data
weight_data <- openxlsx::read.xlsx("/Users/xiaoying/Downloads/Usability-scoringsheet.xlsx")
...
usability_long_data <- usability_long_data %>% 
  mutate(
    x = rep(cumsum(rep(rect_width, 28)), each = 27),
    xmin = x - rect_width/2,
    xmax = x + rect_width/2,
    ...
    ymax = rep(c(1:27) * rect_width + cumsum(width[1:27]), 28),
    ymin = ymax - 0.5,
    y = ymin + 0.25,
  )
...
segment_data <- tibble('x' = c(unique(usability_long_data$x), rep(-5.5, 6)),
                       ...
                       'y' = c(rep(-0.01, 28), unique(usability_long_data$ymax)[c(7, 14, 17, 20, 25, 27)]),
                       'yend' = c(rep(-0.40, 28), unique(usability_long_data$ymin)[c(1, 8, 15, 18, 21, 26)]))
