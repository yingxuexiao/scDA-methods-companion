...
# Bind current process to CPU 18
p = psutil.Process(os.getpid())
p.cpu_affinity([18])
print(f"Bound to CPU core: {p.cpu_affinity()}")

# ================== Cache Directory ==================
cache_dir = Path("/data1_Literature/xiao_DA/conda_cache")
...
data_dir = base_dir / "preprocessed_data"  # Root directory for preprocessed data
...
def process_single_dataset(data_file: Path, celltype_col: str = 'celltype'):
    ...
    # Read data, avoid virtual files (files starting with ._)
    ...
    logger.info(f"End memory: {end_mem:.2f} MB, memory used: {mem_used:.2f} MB")
    ...
    logger.info(f"Chi-square results saved to: {chi2_csv_path}")
